function set_map_view(view_name)
{
    if(view_name == 'poland')
    {
        document.querySelector('svg').viewBox.baseVal.x = -0.254;
        document.querySelector('svg').viewBox.baseVal.y = -0.933;
        document.querySelector('svg').viewBox.baseVal.width = 580;
        document.querySelector('svg').viewBox.baseVal.height = 552; 
    }
    else
    {
        set_map_view('poland');
...
function make_map()
{
    ...
    for (let i=0; i < table_data.length; i++)
    {
        ...
        if (id ==  "")
        {
            console.log("Cannot parse area specification:");
            console.log(row[0]);
            continue; // id not found
        }
        
        
        country_style = "svg ." + id.toUpperCase() +" {fill:" + fill_from_desc(row[1]) + ";}\n";
